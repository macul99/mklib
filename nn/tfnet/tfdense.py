	...
	def build_subnet(inputs, layer_units, training, act, reg, init, args={}, scope=None):
		...
		with tf.variable_scope(scope):
			for i, v in enumerate(layer_units):
				if flag:
					inputs = DenseNet.block(inputs, v, training, act, reg, init, bn_momentum=args['bn_momentum'], 
													drop_rate=args['drop_rate'], scope="block_%d" % (i+1))
				else:
					inputs = DenseNet.block(inputs, v, training, act, reg, init, scope="block_%d" % (i+1))
	...
	def build_backup(inputs, feature_units, stem_units, training, act, reg, init, args={}, scope=None):
		...
		with tf.variable_scope(scope):
			...
			for i, sub_in in enumerate(inputs):
				intermediate_out += [DenseNet.build_subnet(sub_in, feature_units[i], training, act, reg, init, args=args, scope="subnet_%d" % (i+1))]

			net_out = tf.concat(intermediate_out, axis=1, name='intm_concat')

			for i, v in enumerate(stem_units):
				if flag:
					net_out = DenseNet.block(net_out, v, training, act, reg, init, bn_momentum=args['bn_momentum'], 
													drop_rate=args['drop_rate'], scope="stem_%d" % (i+1))
				else:
					net_out = DenseNet.block(net_out, v, training, act, reg, init, scope="stem_%d" % (i+1))
