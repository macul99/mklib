...
class MxLosses:
    ...
    def regression_loss(data, label, num_classes, grad_scale=1.0, act_type='tanh', name="regr"):
        ...
        fc = mx.sym.FullyConnected(data=data, num_hidden=num_classes, name=name+"_fc")
    ...
    @staticmethod
    def arc_landmark_loss(data, mrg_angle, mrg_scale, num_classes, emb_size, landmark_len=10, label_names=['softmax_label', 'landmark_gt'], grad_scales=[1.0, 1.0]):
        ...
        landmark_gt  = mx.symbol.Variable(label_names[1])

        softmax_out = MxLosses.arc_loss(data, softmax_label, num_classes, emb_size, mrg_angle, mrg_scale, grad_scale=grad_scales[0])
        lr_out = MxLosses.regression_loss(data, landmark_gt, landmark_len, act_type='tanh', grad_scale=grad_scales[1])
    ...
    @staticmethod
    def arc_loss_only(data, mrg_angle, mrg_scale, num_classes, emb_size, label_names=['softmax_label'], grad_scales=[1.0]):
        # label_names should match those specified in ImageIter, label_names[0] is for people classification, label_name[1] is for landmark-5p regression
        # landmark_len = num_points*2
