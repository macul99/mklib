...
from tfFaceFeatureExtract import tfFaceFeatureExtract
from caffeFaceFeatureExtract import caffeFaceFeatureExtract


class ExtraceFaceFeature():
    @staticmethod
    def parse_args(self):
        ...
        ap.add_argument("-fw", "--framework", required=True, default='mx', help="framework name") # can be mx, tf or cf
        ap.add_argument("-dlm", "--delimiter", default=' ', help="delimiter")

        ap.add_argument("-mp", "--model-path", required=True, help="model path") #   

        # for mx, tf      
        ap.add_argument("-mon", "--model-outputname", default="", help="model output name") # 

        # for mx
        ap.add_argument("-mpf", "--model-prefix", required=False, help="model prefix") # 
        ap.add_argument("-me", "--model-epoch", required=False, help="model epoch") # 
        ap.add_argument("-mmv", "--model-meanvalue", default="", help="model mean value file") # 

        # for cf
        ap.add_argument("-cfp", "--caffe-proto", required=False, help="caffe model proto") # 
        ap.add_argument("-cfw", "--caffe-weight", required=False, help="caffe model weight") # 
    ...
    def __init__(self, args):
        ...
        assert isdir(self.config.src)

        if not isdir(self.config.dest):
            assert self.config.dest[self.config.dest.rfind('.')+1:] == 'pkl'

        if self.config.framework == 'mx':
            if self.config.model_meanvalue == "":
                model_meanvalue = None
            else:
                model_meanvalue = self.config.model_meanvalue

            if self.config.model_outputname == "":
                self.extractor = mxFaceFeatureExtract(
                                                        self.config.model_path,
                                                        self.config.model_prefix,
                                                        self.config.model_epoch,
                                                        mean_value=model_meanvalue)
            else:
                self.extractor = mxFaceFeatureExtract(
                                                        self.config.model_path,
                                                        self.config.model_prefix,
                                                        self.config.model_epoch,
                                                        outputs_name={'embedding': self.config.model_outputname},
                                                        mean_value=model_meanvalue)
        elif self.config.framework == 'tf':
            if self.config.model_outputname == "":
                self.extractor = tfFaceFeatureExtract(  self.config.model_path)
            else:
                self.extractor = tfFaceFeatureExtract(  self.config.model_path,
                                                        outputs_name={'embedding': self.config.model_outputname})
        elif self.config.framework == 'cf':
        else:
            assert False


    def get_embedding_to_df(self):
        df_rst = pd.DataFrame(columns=['image_path', 'embedding'])
        for i, img_path in enumerate(list(paths.list_images(self.config.src))):
            df_rst = df_rst.append(pd.Series([img_path, self.extractor.getEmbedding(img_path)], index=['image_path', 'embedding']),ignore_index=True)
            print(img_path)
        return df_rst


    def get_embedding_to_txt(self):
        dir_path = self.config.src
        dst_dir = self.config.dest
        ...
        for i, img_path in enumerate(list(paths.list_images(dir_path))):
            ...
            embedding = self.extractor.getEmbedding(img_path)
            print(img_path)
            np.savetxt(dst_txt_path, embedding[np.newaxis,:], delimiter=self.config.delimiter)
        return

    def get_embedding(self):
        if isdir(self.config.dest)
            self.get_embedding_to_txt()
        else:
            df_rst = self.get_embedding_to_df()
            with open(self.config.dest,'wb') as f:
                pickle.dump(df_rst, f)

if __name__ == '__main__':
    args = ExtraceFaceFeature.parse_args()

    eff = ExtraceFaceFeature(args)
    eff.get_embedding()
    ...
    python -m extractFaceFeature    -s /media/macul/black/face_database_raw_data/template_ali_112x112 \
                                    -d /media/macul/black/face_database_raw_data/template_ali_112x112_insight_original \
                                    -fw mx \
                                    -mp /media/macul/black/mxnet_training/r50/insightface-r50-am-lfw \
                                    -mpf model \
                                    -me 0 \
                                    -mon fc1_output
    ...
    python -m extractFaceFeature    -s /media/macul/black/face_database_raw_data/template_ali_112x112 \
                                    -d /media/macul/black/face_database_raw_data/template_ali_112x112_server_train16 \
                                    -fw mx \
                                    -mp /media/macul/black/mxnet_training/r50/server_train16 \
                                    -mpf train_16 \
                                    -me 41 \
                                    -mon embedding_output \
                                    -mmv /media/macul/black/face_database_raw_data/mscelb_from_insightface/mean.json


    python -m extractFaceFeature    -s /media/macul/black/face_database_raw_data/template_ali_112x112 \
                                    -d /media/macul/black/face_database_raw_data/template_ali_112x112_server_train18 \
                                    -fw mx \
                                    -mp /media/macul/black/mxnet_training/r50/server_train18 \
                                    -mpf train_18 \
                                    -me 62 \
                                    -mmv /media/macul/black/face_database_raw_data/mscelb_from_insightface/mean.json
    ...
    python -m extractFaceFeature    -s /media/macul/black/face_database_raw_data/template_ali_112x112 \
                                    -d /media/macul/black/face_database_raw_data/template_ali_112x112_tf_mobilenet \
                                    -fw tf \
                                    -mp /media/macul/black/MobileFaceNet_TF/MobileFaceNet_9925_9680.pb \
                                    -mon prefix
    ...
    python -m extractFaceFeature    -s /media/macul/black/face_database_raw_data/template_ali_112x112 \
                                    -d /media/macul/black/face_database_raw_data/template_ali_112x112_cf_sphereface \
                                    -fw cf \
                                    -mp /media/macul/black/caffe_recognition_models \
                                    -cfp sphereface_deploy \
                                    -cfw sphereface20_ms1m
    '''
'''
import sys
import pickle
